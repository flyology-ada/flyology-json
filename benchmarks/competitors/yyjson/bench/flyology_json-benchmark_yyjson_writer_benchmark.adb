...
procedure Flyology_JSON.Benchmark_YYJSON_Writer_Benchmark is
   ...
   type Prepared_Access is access all YYJSON.Prepared_Document;
   ...
   Large_Raw_Context : aliased YYJSON.Prepared_Document;
   Nested_Context    : aliased YYJSON.Prepared_Document;
   ...
   procedure Write_Batch
     (Iterations : Flyology_Bench.Iteration_Count;
      Value      : out U64)
   is
      Observation : YYJSON.Write_Observation;
   begin
      Value := 0;
      for Iteration in Flyology_Bench.Iteration_Count range 1 .. Iterations loop
         YYJSON.Write (Contexts (Current_Fixture).all, Observation);
         if Observation.Status /= YYJSON.Write_Succeeded then
            raise Program_Error with "yyjson writer rejected its prepared DOM";
         end if;
   ...
   procedure Prepare (Kind : Fixture_Kind) is
      Parse_Result : YYJSON.Parse_Observation;
      Write_Result : YYJSON.Write_Observation;
      Matches      : Boolean;
   begin
      YYJSON.Prepare_Write
        (Fixtures (Kind).Expected_Output.all,
         Contexts (Kind).all,
         Parse_Result);
      if Parse_Result.Status /= YYJSON.Accepted then
         raise Program_Error with "yyjson rejected a writer comparison fixture";
      end if;
      YYJSON.Check_Write_Output
        (Contexts (Kind).all,
         Fixtures (Kind).Expected_Output.all,
         Write_Result,
         Matches);
      if Write_Result.Status /= YYJSON.Write_Succeeded or else not Matches then
         raise Program_Error with "yyjson output policy differs from the comparison fixture";
      end if;
   end Prepare;
   ...
   begin
      YYJSON.Write (Contexts (Kind).all, Observation);
      Current_Fixture := Kind;
      declare
         Benchmark_Name : constant String :=
           "comparison/lane=write_dom/implementation=yyjson/fixture=" & Name (Kind)
   ...
   for Kind in Fixture_Kind loop
      if Selected (Kind) then
         begin
            ...
            YYJSON.Release (Contexts (Kind).all);
         exception
            when others =>
               YYJSON.Release (Contexts (Kind).all);
               raise;
...
end Flyology_JSON.Benchmark_YYJSON_Writer_Benchmark;
