...
procedure Flyology_JSON.Comparison_Benchmark is
   ...
   function Make_Fixture
     (Name          : String;
      ...
      Scalars          : U64;
      Member_Names     : U64;
      YYJSON_Values    : U64) return Fixture is
   ...
   begin
      for Index in 0 .. 65_535 loop
         ...
         case Index mod 4 is
            when 0      => Unbounded.Append (Text, "null");
            when 1      => Unbounded.Append (Text, "true");
            when 2      => Unbounded.Append (Text, "false");
            when others => Unbounded.Append (Text, '0');
         end case;
      end loop;
      Unbounded.Append (Text, ']');
      return Make_Fixture
   ...
   Expected_Signature : constant Signature_Array :=
     [Small_Mixed =>
      ...
      Large_Mixed =>
        [Flyology_Events => 73_814_413_998,
      ...
      Number_Heavy =>
        [Flyology_Events => 21_140_742_174,
      ...
      Long_Mantissa_Numbers =>
        [Flyology_Events => 43_497_177_118,
      ...
      Large_Array =>
        [Flyology_Events => 14_497_054_750,
   ...
   function Supported (Using : Implementation; Kind : Fixture_Kind) return Boolean is
     (not (Using = Serde_JSON_DOM and then Kind = Deep_Nesting));

   function Lane_Name (Using : Implementation) return String is
     (case Using is
         when Flyology_Events | Rapidjson_Events => "parse_events",
         when YYJSON_Validate                    => "parse_validate",
         when others                             => "parse_dom");

   function Implementation_Name (Using : Implementation) return String is
     (case Using is
         ...
         begin
            if Used < Item.Data'Length then
               Core.Next
                 (Parser,
                  Item.Data
                    (Item.Data'First + Offset (Used) .. Item.Data'Last),
                  End_Of_Input => True,
                  Result       => Result);
            else
               declare
            ...
            case Result.Outcome is
               when Core.Event_Ready =>
                  Events := Events + 1;
                  Value := Value + U64 (Core.Event_Kind'Pos (Result.Item.Kind) + 1);
                  Value := Value + U64 (Result.Item.Source.First);
                  Value := Value + U64 (Result.Item.Source.Octet_Length);
               when Core.Document_Complete =>
                  if Used /= Item.Data'Length then
                     raise Program_Error with "Flyology completed before the fixture end";
                  end if;
                  Value := Value xor Events xor U64 (Item.Data'Length) * 11;
                  Event_Count := Events;
                  return;
               when Core.Need_Input | Core.Parse_Failed | Core.Call_Rejected =>
      ...
      case Using is
            ...
            begin
               ...
               then
                  raise Program_Error with
                    "RapidJSON SAX rejected a valid comparison fixture";
               end if;
               Value :=
                 Result.Checksum
               ...
               then
                  raise Program_Error with
                    ...
                    & Ada.Strings.Fixed.Trim
                        (U64'Image (Result.Value_Count), Ada.Strings.Both);
               end if;
               Value :=
                 Result.Checksum
                 xor Result.Value_Count * 3
                 xor Result.Read_Octets * 11;
            end;
         ...
         when Serde_JSON_DOM | Sonic_RS_DOM | SIMD_JSON_DOM =>
            ...
            begin
               ...
               Value :=
                 Result.Checksum
                 xor U64 (Result.Items) * 3
                 xor U64 (Item.Data'Length) * 11;
            end;
      end case;
   end Run_Once;
   ...
   begin
      case Using is
         ...
         when Serde_JSON_DOM | Sonic_RS_DOM | SIMD_JSON_DOM =>
            ...
            begin
               Rust.Parse (Which, Item.Data.all, Result);
               if Result.Status /= Rust.Success
                 or else U64 (Result.Items) /= Item.Logical_Events
               then
   ...
   procedure Measure_Implementation is new Flyology_Bench.Measure_Result_Batched
     (Element => U64, Batch => Run_Batch);
